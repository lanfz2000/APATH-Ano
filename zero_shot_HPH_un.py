...
if __name__ == "__main__":
    ...
    # dataset
    # train_data_root = '/home/ubuntu/data/X/datasets/RINGS/train-all-patch/'
    ...
    idx_un = idx_un.detach().cpu().numpy()
    np.random.shuffle(idx_un)
    # np.random.shuffle(idx_un)
    idx_un = torch.from_numpy(idx_un)
    ...
    # use idx_un
    # print(y_pred.shape, y_true.shape, embeddings.shape)
    ...
    # idx, y_pred = cluster_filter3(args, embeddings.numpy(), y_pred)
    idx, _ = cluster_filter3(args, embeddings.numpy(), y_pred)
    # # print('123', idx)
    ...
    data_df.to_csv('pseudo_data/HPH_un_20_0.3.csv', index=False)
    t2 = time.time()
